<?php
	...
	if(isset($_GET['prefix'])){
        ...
        if(isset( $_GET ['start'] ))
                ...
                $sql = "SELECT `user_id`,`nick`,`school`,`solved`,`submit` FROM `users` $where ORDER BY `solved` DESC,submit,reg_time  LIMIT  " . strval ( $rank ) . ",$page_size";
                ...
                if($scope){
                        ...
                        //echo $s."<-------------------------";
                        $sql="SELECT users.`user_id`,`nick`,`school`,s.`solved`,t.`submit` FROM `users`
                                        inner join
		...
		if(isset($_GET['prefix'])){
			$result = pdo_query($sql,$_GET['prefix']."%");
                ...
                for ( $i=0;$i<$rows_cnt;$i++ ) {
                        ...
                        $view_rank[$i][3]=  htmlentities ( $row['school'] ,ENT_QUOTES,"UTF-8") ;
                        $view_rank[$i][4]=  "<a href='status.php?user_id=" .htmlentities ( $row['user_id'],ENT_QUOTES,"UTF-8") ."&jresult=4'>" . $row['solved']."</a>";
                        $view_rank[$i][5]=  "<a href='status.php?user_id=" . htmlentities ($row['user_id'],ENT_QUOTES,"UTF-8") ."'>" . $row['submit'] . "</a>";

                        if ($row['submit'] == 0)
                                $view_rank[$i][6]= "0.000%";
                        else
                                $view_rank[$i][6]= sprintf ( "%.03lf%%", 100 * $row['solved'] / $row['submit'] );
