...
for ($i=0;$i<$rows_cnt;$i++){
	...
	switch($row['result']){
        ...
        // $view_status[$i][3]="<span class=\"status accepted\"><i class=\"checkmark icon\"></i> <b>Accepted正确</b></span>";
        // $view_status[$i][3]="";
        ...
        if (intval($row['result'])==11 && ((isset($_SESSION[$OJ_NAME.'_'.'user_id'])&&$row['user_id']==$_SESSION[$OJ_NAME.'_'.'user_id']) || isset($_SESSION[$OJ_NAME.'_'.'source_browser']))){
                $view_status[$i][3].= "<a href='ceinfo.php?sid=".$row['solution_id']."' class='".$judge_style[$row['result']]."' >".$judge_icon[$row['result']].$MSG_Compile_Error."";
        	...
        	if ($row['result']!=4&&isset($row['pass_rate'])&&$row['pass_rate']>0&&$row['pass_rate']<.98)
                ...
                $view_status[$i][3].= "<a href='reinfo.php?sid=".$row['solution_id']
					."' class='".$judge_style[$row['result']]."' title='$MSG_Tips'>".$judge_icon[$row['result']].$judge_result[$row['result']]."";
        	if ($row['result']!=4&&isset($row['pass_rate'])&&$row['pass_rate']>0&&$row['pass_rate']<.98)
                                $view_status[$i][3].= (100-$row['pass_rate']*100)."%</a>";
		else
				 $view_status[$i][3].= "</a>";
              ...
              if(!$lock||$lock_time>$row['in_date']||$row['user_id']==$_SESSION[$OJ_NAME.'_'.'user_id']){
                if($OJ_SIM&&$row['sim']>50&&$row['sim_s_id']!=$row['s_id']) {
                        $view_status[$i][3].= "<span class='".$judge_style[/*$row['result']*/17]."'  title='$MSG_Tips'>".$judge_icon[$row['result']].$judge_result[$row['result']]."*";
        		if ($row['result']!=4&&isset($row['pass_rate'])&&$row['pass_rate']>0&&$row['pass_rate']<.98)
				...
				$view_status[$i][3].="</span>";
                        ...
                        $view_status[$i][3].= "<span class='".$judge_style[$row['result']]."'  title='$MSG_Tips'>".$judge_icon[$row['result']].$judge_result[$row['result']]."";
        		if ($row['result']!=4&&isset($row['pass_rate'])&&$row['pass_rate']>0&&$row['pass_rate']<.98)
                                $view_status[$i][3].= (100-$row['pass_rate']*100)."%</span>";
			else
				$view_status[$i][3].="</span>";
