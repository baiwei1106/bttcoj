...
if(count($result)!=1){
  ...
  if(isset($_GET['id'])){
    ...
    if($i=count($result)){
      $view_errors.= "很抱歉，题目已经在比赛中，无法查看！<br>";
      foreach($result as $row){
        $view_errors.= "<a href=contest.php?cid=$row[0]>Contest $row[0]:".htmlentities($row[1],ENT_QUOTES,"utf-8")."</a><br>";
      }
    }else{
      $view_title= "<title>题目不存在!</title>";
      $view_errors.= "很抱歉，您找的题目不存在!";
    }
  }else{
    $view_title= "<title>题目不存在!</title>";
    $view_errors.= "很抱歉，参数错误!";
